...
def translator_destructuring(an_input: str) -> str:
    ...
    while rest:
        ...
        result += [" ".join([str(x) for x in calculate_rover_final_position(rover, rover_movements)])]
    return "\n".join(result)
...
def calculate_rover_final_position(rover: Rover, movements: str) -> List[int]:
    for movement in movements:
        if movement == "L":
            ...
            rover.move()
    return [rover.x_position, rover.y_position, rover.direction_view.value]
